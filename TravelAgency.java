...
public class TravelAgency {
	...
	public List<VarshTourPackage> generatePackageCost() throws InvalidIdException, SQLException {
		List<VarshTourPackage> list = new ArrayList();
		int i = 1;
		Scanner sc = null;
		try {
			...
			File file = new File("B://Package.txt");
			sc = new Scanner(file);
			while (sc.hasNextLine()) {
				...
				if (validate(id)) {
					...
					VarshTourPackage P1 = new VarshTourPackage(id, source_place, destination_place,days,
							basic_fare);
	...
	public List<VarshTourPackage> findPackagesWithMinimumNumberOfDays() {
		...
		try {
			...
			while (rs.next()) {
				...
				VarshTourPackage v1 = new VarshTourPackage(id, src, dstn, days, 0);
